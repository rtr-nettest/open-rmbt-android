...
class TestControllerImpl(
     ...
     * (e.g. [StateRecorder.onQoSTestCompleted]) already treat as "no QoS results". The whole phase
     * runs on a worker thread (not the [GlobalScope.async] coroutine) so a throw there isn't silently
