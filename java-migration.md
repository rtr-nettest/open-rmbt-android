...
**Optimization applied — adaptive I/O batch (cut the syscall rate without hurting slow links):**
The wire protocol works in `chunksize` units, and the original code read/wrote exactly one chunk per syscall, so the syscall rate scaled with throughput (≈ throughput / chunksize — e.g. ~64k read + 64k write syscalls/s per connection at 2 Gbit/s with a 4 KB chunk). A *fixed* large buffer fixes that at high speed but breaks low speed: at ~100 kbit/s a 256 KB write would block for ~20 s, wrecking the sampling/terminate cadence.

So the batch size is now **adaptive**, mirroring the Rust CLI client's pre-test heuristic (`clientRust/src/pretest.rs`): **target ≈ 50 batches/sec (one per ~20 ms), i.e. `batchBytes = bytesPerSecond / 50`**, rounded to a whole number of chunks and clamped to `[chunksize, 256 KiB]`.
- The existing per-thread "short download" pre-test now also estimates throughput: `downloadChunks()` returns the server-reported `TIME` (parsed via `TIME_PATTERN`); the loop keeps the first tiny batch's TIME as an RTT estimate and the largest batch's bytes/TIME, then `computeIoBufferSize(bytes, ns, rttNs)` sets `ioBufferSize = clamp(round_to_chunk(bps/50), chunksize, 256 KiB)`.
- **Download** reads into `downloadBuffer()` sized to `ioBufferSize`. Because one read can now span several chunks, the continue/terminate detection inspects the **last chunk boundary completed by each read** (`newTotal/chunksize > totalRead/chunksize` → check `buffer[lastBoundary - totalRead]`) instead of assuming ≤ one boundary per read.
- **Upload** batches `ioBufferSize / chunksize` continue-chunks per `write()`; the terminating `0xff` chunk is still sent on its own.
- Result: each read/write carries ~20 ms of data at *any* speed → at 100 kbit/s the batch collapses to one `chunksize` chunk (original behaviour, responsive), at multi-Gbit/s it grows to 256 KiB (≈64× fewer syscalls at a 4 KB chunk). Bytes on the wire and chunk-boundary semantics are unchanged.

Note vs. the Rust client: Rust speaks the *newer* protocol — the server announces `CHUNKSIZE <default> <min> <max>` and the client sets the **protocol** chunk size on the wire (`GETTIME <s> <cs>`, `PUTNORESULT <cs>`) while doing socket I/O in fixed 16 KB blocks. This Android client speaks the *older* protocol (server-fixed `chunksize`, `GETTIME <s>` with no size), so the same `bps/50` heuristic is applied to the **client-side I/O batch** instead. Upgrading to client-chosen protocol chunk sizes would require the greeting min/max parse + sized `GETTIME`/`PUT` and server support — a separate, larger change.
