...
## Checkpoint 22 — protocol-level dynamic chunk size

Replaced the interim client-side adaptive I/O batch (see the performance appendix) with the **full Rust-reference protocol-level dynamic chunk size** in `RMBTTest.kt`. The client now parses the server's `CHUNKSIZE <default> <min> <max>` greeting, estimates throughput in the per-thread pre-test, and picks `chunksize = bps/50` (≈ one chunk per 20 ms) clamped to `[min, min(max, 256 KiB)]`, sending it on the wire via `GETTIME <s> <cs>` / `PUTNORESULT <cs>` / `PUT <cs>`. Because the protocol chunk *is* the adaptive unit, the separate batching layer (`ioBufferSize`/`downloadBuffer`/`computeIoBufferSize`) was removed; the upload writer sends one chunk per `write()`. Read loops use generalized chunk-boundary detection (a read may span several chunks). See the updated **Speed-measurement performance** appendix for the full rationale and the one deviation from the Rust client (`PUT <cs>` server-feedback upload + server-default pre-test chunk).
...
**Optimization applied — protocol-level dynamic chunk size (cut the syscall rate without hurting slow links):**
The wire protocol works in `chunksize` units, and the original code read/wrote exactly one chunk per syscall, so the syscall rate scaled with throughput (≈ throughput / chunksize — e.g. ~64k read + 64k write syscalls/s per connection at 2 Gbit/s with a 4 KB chunk). A *fixed* large chunk fixes that at high speed but breaks low speed: at ~100 kbit/s a 256 KB write would block for ~20 s, wrecking the sampling/terminate cadence.

So the **protocol chunk size is now chosen per connection** from a throughput estimate, fully mirroring the Rust CLI client (`clientRust/src/pretest.rs`): **target ≈ 50 chunks/sec (one per ~20 ms), i.e. `chunkSize = bytesPerSecond / 50`**, rounded to the nearest KiB and clamped to the server-announced `[min, max]` (further capped at `MAX_CHUNK_SIZE` = 256 KiB to bound per-thread memory on mobile). The chosen size is sent on the wire, so it is the actual unit the server transfers — no separate client-side batching layer.
- **Greeting parse** — `connect()` now reads `CHUNKSIZE <default> <min> <max>` into `chunksize`/`chunksizeMin`/`chunksizeMax` (older servers send only the default → min = max = default).
- **Pre-test** — the existing per-thread "short download" estimates throughput: `downloadChunks()` (still `GETCHUNKS <n>`, server-default chunk) returns the server-reported `TIME` (parsed via `TIME_PATTERN`); the loop keeps the first tiny batch's TIME as an RTT estimate and the largest batch's bytes/TIME, then `computeChunkSize(bytes, ns, rttNs)` sets `chunksize = clamp(round_to_KiB(bps/50), min, min(max, 256 KiB))` and (re)allocates `buf` to match.
- **Download** sends `GETTIME <s> <chunksize>`; **upload** sends `PUTNORESULT <chunksize>` (pre-test) and `PUT <chunksize>` (main). Because a single socket read can still span several chunks (the kernel coalesces), the continue/terminate detection inspects the **last chunk boundary completed by each read** (`newTotal/chunksize > totalRead/chunksize` → check `buffer[lastBoundary - totalRead]`) instead of assuming ≤ one boundary per read.
- The upload writer (`futureTx`) sends exactly one chunk per `write()`; since the chunk is already ~20 ms of data, the old client-side batching layer (`ioBufferSize`/`downloadBuffer`/`computeIoBufferSize`) is gone.
- Result: each read/write carries ~20 ms of data at *any* speed → at 100 kbit/s the chunk collapses to the server min (responsive), at multi-Gbit/s it grows to the 256 KiB cap (≈64× fewer syscalls vs. a 4 KB chunk). Chunk-boundary semantics (last byte `0x00` continue / `0xff` terminate) are unchanged.

Note vs. the Rust client: this now matches the Rust approach — both parse `CHUNKSIZE <default> <min> <max>` and set the **protocol** chunk size on the wire via `GETTIME <s> <cs>` / `PUTNORESULT <cs>`. One Android-specific detail: the measured upload uses `PUT <cs>` (the server-feedback variant that streams `TIME … BYTES …` back, which drives the Android upload graph) rather than the Rust client's `PUTNORESULT <cs>` client-side measurement; `PUT <cs>` is the symmetric RMBT command. The pre-test itself still uses the server-default chunk (`GETCHUNKS <n>`) for the estimate rather than growing the chunk during the pre-test as Rust does — the estimate is then used to pick the chunk for the measured phases.
