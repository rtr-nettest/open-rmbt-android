...
//            map?.awaitMapLoad()
     ...
     * of sync, so for a brief moment the band still belongs to the previous technology (a transient
     * wrong combination). During the suppression the pill shows just the technology; a delayed refresh
     * re-adds the band when it elapses (even if no new signal update arrives in the meantime).
     */
